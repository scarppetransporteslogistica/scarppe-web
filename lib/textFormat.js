...
// Admin > General's site-wide typography scale is implemented as compound
// two-class Tailwind selectors (e.g. `.font-heading.text-4xl`) with
// !important, so it can reach every heading/paragraph without the admin
// having to configure anything. But when BOTH that global rule and a
// per-block/per-section override below also use !important on the same
// property, CSS breaks the tie by SPECIFICITY first (not source order) —
// and a single-class selector like `.tf-my-id` (specificity 0,0,1,0) always
// LOSES to a two-class selector (0,0,2,0), no matter which one appears
// later in the document. That silently made every per-block size/family/
// color override in this file inert whenever the element also carried the
// global font-heading/font-body + text-size classes (i.e. almost always).
// Repeating the same class 3× in the selector is valid CSS and keeps
// matching the exact same single class on the element, but raises the
// selector's specificity to 0,0,3,0 — unconditionally higher than any
// 2-class global rule, so the more specific, more recently-set per-block
// override always wins as intended.
function boostSel(className) {
  const c = `.${className}`;
  return `${c}${c}${c}`;
}
...
function boxDeclarationsFor(fmt) {
  ...
  // `posX`/`posY` are the older field names used by the gallery's "zoom /
  // encuadre" feature before it got per-device controls — read as aliases
  // so any crop already saved that way keeps looking the same.
  const rawPosX = fmt.objectPositionX !== undefined && fmt.objectPositionX !== "" && fmt.objectPositionX !== null ? fmt.objectPositionX : fmt.posX;
  const rawPosY = fmt.objectPositionY !== undefined && fmt.objectPositionY !== "" && fmt.objectPositionY !== null ? fmt.objectPositionY : fmt.posY;
  const hasPosX = rawPosX !== undefined && rawPosX !== "" && rawPosX !== null;
  const hasPosY = rawPosY !== undefined && rawPosY !== "" && rawPosY !== null;
  if (hasPosX || hasPosY) {
    const x = hasPosX ? Number(rawPosX) : 50;
    const y = hasPosY ? Number(rawPosY) : 50;
    decls.push(`object-position:${x}% ${y}% !important`);
  }
  // Manual photo zoom (used by the gallery "zoom / encuadre" feature),
  // independent per device like everything else here — scales the image
  // from the same point as its crop position, so a photo can be zoomed in
  // more (or less) on mobile than on desktop without affecting the other.
  if (fmt.zoom !== undefined && fmt.zoom !== "" && fmt.zoom !== null && Number(fmt.zoom) !== 100) {
    const zx = hasPosX ? Number(rawPosX) : 50;
    const zy = hasPosY ? Number(rawPosY) : 50;
    decls.push(`transform:scale(${Number(fmt.zoom) / 100}) !important`);
    decls.push(`transform-origin:${zx}% ${zy}% !important`);
  }
  return decls.length ? decls.join(";") : null;
}
...
export function sectionTypographyCSS(className, cfg) {
  if (!cfg) return "";
  let css = "";
  // Boosted the same way as boostSel() above: repeating .font-heading/
  // .font-body 3x raises specificity past the global scale's 2-class rules.
  const sel = `.${className} .font-heading.font-heading.font-heading, .${className} .font-body.font-body.font-body`;
...
// intensity, how far up/across it extends, and direction). Opt-in and
// PER DEVICE-CONTEXT independent: configuring a custom gradient for, say,
// celular must never touch escritorio/tablet's original hand-tuned look.
// Each of the 6 contexts (desktop + 5 media-scoped) is checked and resolved
// completely on its own — there is no shared "base" fallback between them,
// unlike the text/box engines, because Hero's true default isn't a single
// look that just scales, it's a genuinely different composition per group
// (mobile: 3-stop vertical scrim; tablet+desktop: 5-stop horizontal scrim;
// plus an always-on bottom vignette). A context with no configured fields
// simply emits no rule at all, so Hero.js's original hardcoded layer for
// that context keeps showing through untouched.
...
const GRADIENT_CONTEXT_KEYS = ["desktop", "tabletPortrait", "tabletLandscape", "mobilePortrait", "mobileLandscape", "wide"];
const GRADIENT_MEDIA = {
  ...MEDIA_QUERIES,
  // Desktop needs its own explicit (mutually-exclusive) query here too —
  // unlike the text engine, "desktop" is not an unconditional fallback for
  // this feature, so it must be scoped just like every other context.
  desktop: "(min-width:1180px) and (not ((orientation:landscape) and (max-width:1366px) and (max-height:1024px)))",
};
...
function contextGradientFields(cfg, key) {
  if (!cfg) return {};
  return key === "desktop" ? ownGradientFields(cfg) : ownGradientFields(readContext(cfg, key));
}

// True if ANY context has ANY field set — used only to decide whether to
// show the "Personalizado" style summary in the admin, not for rendering.
export function hasGradientOverride(cfg) {
  if (!cfg) return false;
  return GRADIENT_CONTEXT_KEYS.some((key) => Object.keys(contextGradientFields(cfg, key)).length > 0);
}

function resolveContextGradient(cfg, key) {
  return { ...GRADIENT_DEFAULTS, ...contextGradientFields(cfg, key) };
}
...
// `className`: the always-present custom overlay div (transparent unless a
// context below gives it a background). `hideSelectors`: which original
// default-layer class(es) to hide, PER CONTEXT — e.g.
// { mobilePortrait: ".hero-ov-mobile", mobileLandscape: ".hero-ov-mobile",
//   tabletPortrait: ".hero-ov-td", tabletLandscape: ".hero-ov-td",
//   desktop: ".hero-ov-td", wide: ".hero-ov-td" }
// so only the layer that would otherwise show at that exact breakpoint gets
// hidden — every other device's default stays exactly as it was.
export function heroGradientCSS(className, cfg, hideSelectors = {}) {
  let css = "";
  const sel = boostSel(className);
  for (const key of GRADIENT_CONTEXT_KEYS) {
    if (!Object.keys(contextGradientFields(cfg, key)).length) continue;
    const resolved = resolveContextGradient(cfg, key);
    const hide = hideSelectors[key];
    const hideRule = hide ? `${hide}{display:none !important}` : "";
    css += `@media ${GRADIENT_MEDIA[key]}{${hideRule}${sel}{background:${gradientBackground(resolved)} !important}}`;
  }
  return css;
}
