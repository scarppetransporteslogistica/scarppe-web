export function buildThemeVars(settings) {
  ...
  const lines = [
    ...
    // Line-height ("interlineado"), same percentage/multiplier pattern.
    `--heading-line-height: ${(Number(typo.headingLineHeightPercent) || 100) / 100};`,
    `--body-line-height: ${(Number(typo.bodyLineHeightPercent) || 100) / 100};`,
...
export function googleFontsHref(fonts, extraFamilies = []) {
  const families = Array.from(new Set([fonts?.heading, fonts?.body, ...extraFamilies].filter(Boolean)));
  // Request a full weight range (100-900) plus a couple of italic weights so
  // variable fonts like Outfit render all their variations correctly.
  const query = families
    .map(
      (f) =>
        `family=${encodeURIComponent(
          f
        )}:ital,wght@0,100;0,200;0,300;0,400;0,500;0,600;0,700;0,800;0,900;1,300;1,400;1,700`
    )
    .join("&");
  return `https://fonts.googleapis.com/css2?${query}&display=swap`;
}

// Recursively walks any content object/array and collects every
// `fontFamily` value found (per-section typography, per device/orientation
// sub-objects, etc.) so those fonts actually get requested from Google
// Fonts — not just the site-wide heading/body fonts set in Admin > General.
export function collectFontFamilies(node, acc) {
  const set = acc || new Set();
  if (!node || typeof node !== "object") return set;
  if (typeof node.fontFamily === "string" && node.fontFamily) set.add(node.fontFamily);
  for (const value of Object.values(node)) {
    if (value && typeof value === "object") collectFontFamilies(value, set);
  }
  return set;
}
